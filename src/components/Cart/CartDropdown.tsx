...
import { CartProduct, MINICART_PRODUCT } from '../../common/gql'
...
import MiniCartProductView                      from './MiniCartProduct'

type Props = { currency: string, onRedirect: () => void }
type State = { cart: CartProductType[], prices: number[] }
...
class CartDropdown extends React.Component<Props, State> {
	...
	price = (product: CartProduct) => this.props.currency +
			...
			<div className='cartOverlay'>
				...
				<div className='items'>
					{this.state.cart.map((prod, index) => {
						...
						return <Query query={MINICART_PRODUCT} variables={{productId: prod.productRecord.id}}
							key={key}
							onCompleted={(data: { product: CartProduct }) => {
						...
						{(result: QueryResult<any>) => {
							...
							const product: CartProduct = data.product

							return <MiniCartProductView
								key={key}
								id={key}
